...
@php
    ...
    $sparkLabels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
    $totalSpark = [1400, 1600, 1525, 1780, 1900, 2140];
    $recognizedSpark = [900, 980, 1040, 1080, 1180, 1260];
    $fundingSpark = [320, 360, 390, 420, 460, 520];
    $jobsSpark = [4200, 4450, 4680, 4900, 5150, 5480];

    $months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    $registrations = [112, 128, 140, 165, 178, 190, 210, 222, 235, 248, 260, 279];
    $sectors = [
    ...
    $stateFunding = [480, 455, 410, 388, 365, 342];
    $fundingSeries = [260, 290, 320, 360, 402, 450, 498, 560, 612, 685, 742, 810];
...
<section class="space-y-6">
    ...
    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        <x-ui.kpi-card title="Total Startups" value="18,420" trend="14.2%" direction="up" icon="rocket" :spark-labels="$sparkLabels" :spark-values="$totalSpark" description="Across registered, active, and in-review profiles." />
        <x-ui.kpi-card title="DPIIT Recognised" value="12,860" trend="9.8%" direction="up" icon="shield-check" :spark-labels="$sparkLabels" :spark-values="$recognizedSpark" description="Recognition approvals moving steadily upward." />
        <x-ui.kpi-card title="Funding Raised" value="Rs 4,820 Cr" trend="18.4%" direction="up" icon="funding" :spark-labels="$sparkLabels" :spark-values="$fundingSpark" description="Tracked across grants, seed, and growth rounds." />
        <x-ui.kpi-card title="Jobs Created" value="2.41 L" trend="11.6%" direction="up" icon="users" :spark-labels="$sparkLabels" :spark-values="$jobsSpark" description="Direct employment linked to startup operations." />
    </div>
    ...
    <div class="grid gap-6 xl:grid-cols-2">
        <x-ui.chart-card title="Monthly startup registrations" subtitle="New registrations and profile updates over the last twelve months.">
            <canvas data-chart="line" data-labels='@json($months)' data-datasets='@json([["label" => "Registrations", "data" => $registrations, "borderColor" => "#4f46e5", "backgroundColor" => "rgba(79, 70, 229, 0.12)", "tension" => 0.42, "fill" => true]])'></canvas>
        </x-ui.chart-card>

        <x-ui.chart-card title="Sector distribution" subtitle="Share of active startups by dominant sector.">
            <canvas data-chart="doughnut" data-labels='@json(array_column($sectors, "label"))' data-datasets='@json([["data" => array_column($sectors, "value"), "backgroundColor" => array_column($sectors, "color")]])'></canvas>
        </x-ui.chart-card>
