...
<div class="space-y-6">
    ...
    <form class="space-y-4" method="POST" action="{{ route('auth.login.store') }}">
        @csrf
        ...
        <x-ui.auth-field name="password" label="Password" type="password" autocomplete="current-password" toggle="true" :error="$formErrors->first('password')" />

        <div class="flex items-center justify-between gap-3 text-sm">
            <label class="flex items-center gap-2 text-slate-300">
                <input name="remember" type="checkbox" class="h-4 w-4 rounded border-slate-600 bg-slate-900 text-indigo-500 focus:ring-indigo-500" {{ old('remember') ? 'checked' : '' }}>
                Remember me
            </label>
