...
function updatePreferenceStatus(key, enabled) {
    const chips = document.querySelectorAll(
        `[data-setting-status="${key}"], [data-setting-summary="${key}"]`,
    );
    ...
    document
        .querySelectorAll(`[data-setting-display="${key}"]`)
        .forEach((input) => {
...
function updateThemeSelection(theme) {
    ...
    document
        .querySelectorAll("[data-theme-status], [data-theme-summary]")
        .forEach((chip) => {
            const active =
                theme === "dark"
                    ? chip.matches("[data-theme-status]") ||
                      chip.matches("[data-theme-summary]")
                    : chip.matches("[data-theme-summary]");
...
function setSectorModalView(modal, view) {
    ...
    modal.querySelectorAll("[data-sector-view-panel]").forEach((panel) => {
        panel.classList.toggle(
            "hidden",
            panel.getAttribute("data-sector-view-panel") !== view,
        );
    });
}
...
function initFlashMessages() {
    const dismissFlashMessages = () => {
        document
            .querySelectorAll("[data-flash-message]")
...
function initSettingsPreferences() {
    ...
    updateThemeSelection(
        root.getAttribute("data-theme") || getPreferredTheme(),
    );
}
